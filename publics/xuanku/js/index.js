...
miaov.currentStep = "step1";//上来先默认从第一个开始
...
miaov.events = function(){
	...
	$(window).bind("scroll",scrollFn);//绑定滚动事件。避免上来触发滚定事件
	...
	var timer = null;
	function mousewheelFn(ev,direFFHction){
		$(window).unbind("scroll",scrollFn);
		if( direFFHction<1 ){  //向下滚动
			//console.log("next");
...
miaov.scrollStatus = function (){
	var times = miaov.scale() * miaov.timeScroll.totalDuration();//总时间
	//当滚动条在滚动的过程中，让页面中的动画到打某个时间点
	miaov.timeScroll.seek(times,false);//直接运动到那个地方
}
